...
def register():
	...
	with open("log.txt", 'a') as f:
		#content = f.read()
		#f.seek(0, 0)
		f.write(id + ' REGISTERED at '+ str(datetime.utcnow()) +'\n')
	f.close()
	return render_template('rs.html', )
...
def login():
	...
	with open("log.txt", 'a') as f:
		#content = f.read()
		#f.seek(0, 0)
		f.write(userid + ' LOGGED IN at '+ str(datetime.utcnow()) +'\n')
	f.close()
...
def search():
	...
	with open("log.txt", 'a') as f:
		f.write(userid + 'SEARCHED '+ query + ' at ' +str(datetime.utcnow()) +'\n')
	f.close()
	import subprocess
	subprocess.run('cd main && python web_crawl.py '+query.replace(" ", "")+' '+userid+' && cd..',shell=True)
	...
	df = pd.read_csv('main/Search/'+userid+'.csv')
	if len(papers)<15:
		rank = mcda(df)
		results.clear()
		results = []
...
def read(num):
	...
	with open("log.txt", 'a') as f:
		#content = f.read()
		#f.seek(0, 0)
		f.write(userid + ' READ paper: '+ title +' at ' +str(datetime.utcnow()) +'\n')
	f.close()
	db = mysql.connector.connect(
	  host="localhost",
	  user="root",
